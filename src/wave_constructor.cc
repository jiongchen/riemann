...
int wave_constructor::init() {
  ...
  alpha_ij_ = zeros<double>(1, edges_.size(2));
  beta_ij_ = zeros<double>(1, edges_.size(2));
  c_ij_ = zeros<double>(4, edges_.size(2));
...
int wave_constructor::prepare() {
  buff_.resize(3);
  buff_[0] = std::make_shared<wave_value_cons>(edges_, f_, c_ij_, 1.0);
